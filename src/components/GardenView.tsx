  ...
  return (
    <div className="w-full rounded-3xl bg-white/70 backdrop-blur shadow-soft p-4">
      ...
      <div className="flex items-center gap-3 text-sm mb-3 flex-wrap">
        <span className="rounded-full bg-[#FFF1F6] px-3 py-1">🌸 ×{garden.flowers}</span>
        <span className="rounded-full bg-[#FFF8E5] px-3 py-1">☀️ ×{garden.sun}</span>
        <span className="rounded-full bg-[#EAF6FF] px-3 py-1">💧 ×{garden.water}</span>
      </div>
      <div className="rounded-2xl bg-gradient-to-b from-[#F0FFF4] to-[#FFF7FB] min-h-[160px] p-3 relative overflow-hidden">
        <div className="absolute top-2 right-3 flex gap-1">
          {Array.from({ length: sunCount }).map((_, i) => (
            <span key={`s${i}`} className="text-xl animate-soft-float">☀️</span>
          ))}
        </div>
        ...
        <div className="absolute bottom-2 left-3 flex gap-1">
          {Array.from({ length: waterCount }).map((_, i) => (
            <span key={`w${i}`} className="text-lg">💧</span>
          ))}
        </div>
      </div>
