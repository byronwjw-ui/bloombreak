...
export const MATCH_LEVELS_V2: MatchLevel[] = [
  {
    id: 1, name: '周一早会生存战', subtitle: '先把会议气泡清一清', mood: 'meeting', moves: 28,
    goals: [
    ...
    difficulty: '轻松',
    stars: { two: 1500, three: 2400 },
    tip: '会议可以很多，但你不用把每一句都放进脑子里。',
  },
  {
    id: 2, name: '收件箱清零计划', subtitle: '先清掉最近的三封', mood: 'messages', moves: 26,
    goals: [
    ...
    difficulty: '轻松',
    stars: { two: 1800, three: 2800 },
    tip: '先清掉最近的三封，不用一次回复整个世界。',
  },
  {
    id: 3, name: 'Deadline 退散', subtitle: '一块一块拆', mood: 'deadline', moves: 26,
    goals: [
    ...
    difficulty: '轻松',
    stars: { two: 2200, three: 3300 },
    tip: 'Deadline 会叫，但你可以一步一步拆。',
  },
  {
    id: 4, name: 'KPI 不要追我', subtitle: '会议泡泡来了', mood: 'kpi', moves: 27,
    goals: [
      { type: 'clearPressure', tileType: 'kpi', target: 6 },
      { type: 'clearBlocker', blockerType: 'meeting_bubble', target: 3 },
    ],
    weights: { ...baseW, kpi: 7, deadline: 2 },
    blockers: { types: ['meeting_bubble'], count: 4 },
    difficulty: '有点忙',
    stars: { two: 2200, three: 3400 },
    tip: '会议泡泡要相邻匹配两次才能戳破。',
  },
  {
    id: 5, name: '拒绝精神内耗', subtitle: '把雾散一散', mood: 'burnout', moves: 30,
    goals: [
      { type: 'clearPressure', tileType: 'fog', target: 6 },
      { type: 'clearBlocker', blockerType: 'meeting_bubble', target: 3 },
    ],
    weights: { ...baseW, leaf: 14, fog: 7 },
  ...
  {
    id: 6, name: '下班前的最后一杯咖啡', subtitle: '撑过这一小关', mood: 'relax', moves: 26,
    goals: [
      { type: 'clearTile', tileType: 'coffee', target: 20 },
      { type: 'createSpecial', target: 1 },
    ],
    weights: { ...baseW, coffee: 16, meeting: 2, deadline: 2 },
    difficulty: '有点忙',
    stars: { two: 2400, three: 3800 },
    tip: '4 连或 5 连可以做出特殊方块。',
  },
  {
    id: 7, name: '老板突然找你', subtitle: '雾里也要保持节奏', mood: 'messages', moves: 27,
    goals: [
      { type: 'clearPressure', tileType: 'meeting', target: 9 },
      { type: 'clearBlocker', blockerType: 'fog_layer', target: 4 },
    ],
    weights: { ...baseW, meeting: 8, deadline: 4 },
    blockers: { types: ['fog_layer'], count: 5 },
    difficulty: '压力上来了',
    stars: { two: 2800, three: 4200 },
    tip: '雾覆盖的格子需要相邻消除来散开。',
  },
  {
    id: 8, name: '需求又变了', subtitle: '便签 + 锁', mood: 'burnout', moves: 28,
    goals: [
      { type: 'clearTile', tileType: 'note', target: 18 },
      { type: 'clearBlocker', blockerType: 'kpi_lock', target: 3 },
    ],
    weights: { ...baseW, note: 14, fog: 6, meeting: 3 },
    blockers: { types: ['kpi_lock', 'fog_layer'], count: 6 },
    difficulty: '压力上来了',
    stars: { two: 2800, three: 4400 },
    tip: '锁的格子需要相邻匹配解开。',
  },
  {
    id: 9, name: '今天也要偷偷开花', subtitle: '稳住节奏', mood: 'relax', moves: 28,
    goals: [
      { type: 'createSpecial', target: 2 },
      { type: 'score', target: 3000 },
    ],
    weights: { ...baseW, focus: 12, meeting: 3, deadline: 3 },
    blockers: { types: ['fog_layer', 'kpi_lock'], count: 5 },
    difficulty: '压力上来了',
    stars: { two: 3200, three: 4800 },
    tip: 'T/L 形状的消除可以做出冲击波。',
  },
  {
    id: 10, name: '消息轰炸防御战', subtitle: '红点不会等你', mood: 'messages', moves: 28,
    goals: [
      { type: 'clearTile', tileType: 'mail', target: 22 },
      { type: 'clearPressure', tileType: 'meeting', target: 6 },
      { type: 'clearBlocker', blockerType: 'deadline_timer', target: 2 },
    ],
    weights: { ...baseW, mail: 16, meeting: 6, deadline: 3 },
    blockers: { types: ['deadline_timer', 'fog_layer'], count: 5 },
    difficulty: '差一点就下班',
    stars: { two: 3400, three: 5000 },
    tip: '倒计时归零会扣 1 步，先清掉它。',
  },
  {
    id: 11, name: '高效但不燃尽', subtitle: '认真工作，也认真休息', mood: 'deadline', moves: 30,
    goals: [
      { type: 'clearPressure', tileType: 'deadline', target: 6 },
      { type: 'clearPressure', tileType: 'kpi', target: 6 },
      { type: 'createSpecial', target: 2 },
    ...
    blockers: { types: ['deadline_timer', 'kpi_lock'], count: 6 },
    difficulty: '差一点就下班',
    stars: { two: 3800, three: 5400 },
    tip: '你可以认真工作，也可以认真休息。',
  },
  {
    id: 12, name: '给自己留一点光', subtitle: '今天不一定完美', mood: 'relax', moves: 32,
    goals: [
      { type: 'clearTile', tileType: 'focus', target: 18 },
      { type: 'clearBlocker', blockerType: 'fog_layer', target: 6 },
      { type: 'score', target: 4500 },
    ],
    weights: { ...baseW, focus: 12, meeting: 3, deadline: 3, fog: 4, kpi: 3 },
    blockers: { types: ['fog_layer', 'meeting_bubble', 'deadline_timer'], count: 8 },
    difficulty: '差一点就下班',
    stars: { two: 4500, three: 6500 },
